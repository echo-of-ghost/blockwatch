...
### Added
- **The test suites now live in the repo.** They were written across the 3.0.0 work and kept in a temp directory, which was wiped twice during development — every regression caught during that work (pointer-capture ordering, `contain: paint` clipping resize handles, CSS animations hijacking the engine's transform, a leaked resize listener per `start()`, a half-engaged `engage()` blanking the dashboard) was caught by these and by nothing else. `npm test` runs the two headless suites — 50 checks on explorer URL validation and 235 colour pairings through the WCAG gate — in about a second with no node and no window. `npm run test:app` adds 129 browser checks, launching and stopping the Electron app itself. Everything runs locally against your own node; there is no CI wiring.
- **Coverage the suites did not have:** a structural invariant probe (no overlapping panels, nothing outside the data area, column height fractions summing correctly, nothing below `MIN_HF`) asserted after every interaction; the north resize handle; a viewport change mid-drag; corrupt, truncated and wrong-shaped persisted layouts; independence from the panel count; and hostile explorer URLs that carry a hostname, such as `javascript://host/%0aalert(1)` — every previous hostile case was rejected by the hostname guard rather than the scheme check, so deleting the scheme check still left the suite reporting 44 of 46 green.
- **Panels can be hidden from the keyboard.** `Alt`+`H` hides the focused panel and says where it went; the restore bar already brought it back. Hiding was previously reachable only by right-clicking a header, which made it the one layout action with no keyboard path at all.
- **`c` copies the focused row's hash or address.** Clicking a block hash already copied it, but the cell is deliberately not a tab stop — twenty-four rows would put twenty-four stops back into the tab order — so this is how the same action reaches the keyboard.
- **The context menu is a real menu.** Arrow keys move through it, `Enter` activates, `Home` and `End` jump, `Esc` closes and returns focus to the header. It previously had no role, no focusable items and no key handling: every action in it was mouse-only.
- **States the interface did not previously have:** an explicit *Unreachable* badge when Bitcoin Core stops answering, an explanation when fee estimates are unavailable rather than a bare dash, "no peers connected" and "no blocks yet" where a table used to render nothing at all, a fade telling you a table scrolls sideways, and "collecting mempool history…" for the ten seconds before that chart has two points to draw.

### Fixed
- **Running the tests killed the developer's own dashboard.** `stop.sh` matched processes by command line, so `pkill -f .../electron` reached every Electron app started from source — the running dashboard included, and occasionally the shell running the tests. The app is now launched under a recorded pid and a disposable `--user-data-dir`, and only that pid and its children are ever signalled. The scratch profile also means a test can no longer read or destroy the layout saved in `bw_layout_v44`. Verified by running the suites alongside a second dashboard and confirming it survived.
...
- **Hiding a panel could survive a layout reset, and hiding it twice corrupted the column.** `minimize()` does its bookkeeping — marking the panel hidden, redistributing its height among the rest — inside the hide animation's completion callback, and that callback was an uncancellable 180ms `setTimeout`. Two consequences followed. Resetting the layout, or restoring the panel, while the animation was still running left the callback to fire afterwards and hide a panel the user had just brought back; the layout engine then solved for nine panels instead of ten. And calling `minimize()` twice on the same panel inside that window queued two timers, so the redistribution ran twice: the column's remaining panels summed to 1.16 instead of 1 and the survivor was laid out 16% taller than its column. The timer is now tracked per panel — starting a hide cancels any pending one — and both `restore()` and `_reset()` abandon a hide that is still in flight.
- **Duplicate `[Unreleased]` sections** in this file, merged into one.

- **The dashboard invented a healthy node when Bitcoin Core was unreachable.** With bitcoind refusing connections the titlebar read "Syncing", block height read 0, peers 0, "0.00% SYNCED", and the mining panel showed a retarget projection extrapolated from no data whatsoever. Nothing said anything was wrong. The server answers with an error field and zero-filled structures when it cannot reach Core, and the client never looked at the error — it rendered the zero-fill as measurements. For a tool whose only job is reporting the state of a node, confidently reporting the wrong state is the worst failure available to it. The panels are now left alone, so the last values that were actually true stay on screen, and the reason is named: refused, timed out, credentials rejected, or still warming up.
- **Reaching the peer filter from the titlebar took about eighty presses of Tab.** Both tables already moved between rows with the arrow keys, but every row was also a tab stop — 52 of the 125 in the whole application. A data table is one stop now, not one per row, and the stop follows your selection so re-entering a table lands where you left.
- **The settings dialog said it was modal and was not.** Tab walked straight out of it into the dashboard behind. Its focus trap counted the three explorer radios as three stops; a radio group is one, so the boundary it was watching for could never be reached.
- **Copying was mouse-only in four of the five places it is offered.** The key handler for it already existed and simply could never fire, because the elements could not take focus.
- **The terminal was a dead end when the dashboard is served to a browser.** It sends RPC through the desktop app, so in remote mode every command — `help` included — failed with a raw JavaScript type error, while the button, the shortcut and the shortcuts sheet all still advertised it. It now explains what is missing and where it does work.
- **Content was silently deleted at laptop widths.** Panel bodies do not scroll sideways, so a value too wide for its box was not scrolled to, it was gone: at 1100px the node panel lost 64px of the RPC endpoint off the right-hand edge. Values truncate now and keep the full text on hover. The peers table hid 102px of itself at the same width with nothing to say so.
- **Two panels disagreed about the same number.** At the same instant the hero read "5m 7s ago" and the node panel "5m 5s ago" for time since the last block: three places computed it, and only two of them ticked between polls. They share one clock now.
- **"bitcoind unreachable" was blamed for things bitcoind had not done.** The stale-data warning fired whenever no update had arrived for a minute, which could equally be this server or a dropped event stream, and then dismissed itself after five seconds while the condition continued. It now says what is actually known, stays up while it is true, and confirms recovery.
- **The charts were invisible to assistive technology,** carrying no role and no accessible name, so a screen reader got nothing at all from the primary content of three panels.
- **The large numbers shifted sideways as they updated.** They are set in Geist, which is proportional: at 22px "111,111" renders 45.2px wide and "000,000" renders 88.2px. Block height, mempool count, peers and hashrate all tick constantly, so the biggest text on the dashboard moved every time. Tabular figures hold the digit advance without changing the typeface.
...
- **Columns reclaim horizontal space.** Column widths were a fixed constant, so emptying a column left a dead band the width of that column and nothing grew to fill it. A column with no panels now collapses to zero and its width is shared among the rest in proportion to their base weights, animated by the same solver so the reclaim flows rather than snaps. Drop targeting still uses the full four-column geometry, because hit-testing the collapsed layout would make an emptied column zero-width and impossible to move back into.

### Fixed
- **Hiding or restoring a panel made it flash at the top-left corner.** The classic `p-spawn`, `p-restore` and `p-out` keyframes all animate `transform`, and a CSS animation outranks an inline style. In the fluid engine position *is* the inline transform, so for the length of each animation the keyframes replaced it with a bare scale and dropped the panel to the container origin: 10 frames on hide, 16 on restore, measured. In fluid mode those animations now fade only. Restoring also placed the panel synchronously before it becomes visible, since `restore()` sets `display` before any paint can run.
- **Hidden panels could become unrecoverable.** The restore bar clipped its chips at `max-width: 40vw` with `overflow: hidden`, so on a 1280px window hiding nine panels left two with no chip visible and no other way back. The chip strip now scrolls, with a fade marking that there is more and a plain wheel scrolling it, since a horizontal scroll otherwise needs shift and nobody guesses that.
...
- **A successful UTXO set scan deleted its own control.** The scan button lived inside the element the result is written to, so `el.textContent = …` on success destroyed the button. You could scan exactly once per launch, and the figure you were left with carried no timestamp even though the UTXO set changes with every block. The result now renders into a sibling element, the control returns as `rescan`, and the figure shows how long ago it was taken. The request also gained a deadline, since one that never settled left the control disabled for the rest of the session. The failure path already recovered correctly, which is why the working case was the broken one.
- **Service badges were reachable by mouse only.** The WITNESS, NETWORK_LIMITED and P2P_V2 descriptions were revealed by a `mouseover` handler on spans with no `tabindex` and no `title`, so keyboard users could not reach them, touch users got nothing, and screen readers never saw them. The badges are now focusable, the description line responds to focus as well as hover, and the text reaches screen readers as real content. Deliberately no `title`: this panel already renders the description into its own styled line, so a native tooltip would repeat it in a second, unstyled, delayed channel.
...
### Fixed
- **Block links on regtest pointed at mainnet.** The chain-prefix map covered signet and testnet4 but not regtest, so a regtest block hash was appended to a mainnet URL and the link was always dead. Chains the chosen explorer cannot serve now render as plain text instead of a broken link.
- **The overlay scrim rendered as a light halo.** A radial gradient lightened the centre and read as a lamp behind the sheet. It is now a uniform scrim with a layered shadow.

### Security
- **A user-supplied explorer URL reaches an `href`, so it is validated before it can.** Only `http:` and `https:` survive; the check reads the *parsed* protocol rather than the raw string, which is what catches `java\nscript:alert(1)` — URL parsing strips the newline before reporting the scheme, and so does HTML attribute parsing, so a prefix check would have passed it straight to the DOM. Anything unusable fails closed to no link rather than to a guess. Block hashes remain constrained to 64 hex characters, and a corrupt or hand-edited stored config falls back to the default.
...
- **The app waited on a full snapshot before showing anything**: the HTTP listener only started after the first complete state snapshot was built, and Electron only creates its window once the server is listening, so a node answering slowly left the app with no window at all. On a mainnet node mid-IBD a single `getblockchaininfo` was measured at 6.7s and `getchaintips` at 3.9s. The listener now starts immediately and the snapshot fills in behind the connecting overlay the client already had but never got to display.
- **First paint no longer waits for the slowest call**: `initState` ran five sequential round trips and published nothing until the last one returned. It now broadcasts as each stage lands — chain state first, then blocks, then peers and the rest — and the peer batch runs alongside the block fetch rather than before it. Against a node with 2.5s of latency per call, the window went from 30.3s to 0.07s and the height from 30.3s to 2.6s; against a real node mid-IBD the dashboard is fully populated in 0.83s.
- **Desktop window association**: the packaged `.desktop` filename did not match Electron's app id, so desktop environments could show a generic icon for the running window and fail to group it with its launcher. On Wayland the app id must equal the desktop file basename for association to work at all. The entry is now `Blockwatch.desktop`, matching `StartupWMClass=Blockwatch`.
